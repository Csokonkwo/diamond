...
if(isset($_POST['deposit'])){
    ...
    if($_POST['amount'] <= 29){
        $errors['amount'] = "Minimum Deposit allowed is $30";
    }

    //if(strlen($_POST['transAdd']) < 14){
       // $errors['transAdd'] = "Please enter a valid transaction hash";
    //}

    if(strlen($_POST['currency']) < 7){
        $errors['currency'] = "Please choose a payment method";
    }
...
if(isset($_POST['invest'])){
    ...
    if($_POST['amount'] <= 29){
        $errors['amount'] = "Minimum Investment allowed is $30";
    }
    ...
    if (count($errors) === 0){
        ...
        if($firstInvestment == 0){
            if(strlen($_SESSION['referrer_id']) > 4){
