<div class="top-header clearfix">
        <p>Welcome to <?php echo $companyName;?></p>
        <ul>
            <li><a href="#"><i class="fa fa-facebook"></i></a></li>
            <li><a href="#"><i class="fa fa-twitter"></i></a></li>
            <li><a href="#"><i class="fa fa-google-plus"></i></a></li>
            <li><a href="#"><i class="fa fa-linkedin"></i></a></li>
        </ul>
    </div>
    
    <header>
        <img src="<?php echo BASE_URL.'/img/logo.png'?>">
        
        <ul class="nav clearfix">
            <li><a href="<?php echo BASE_URL.'/index.php'?>">Home</a></li>
            <li><a href="<?php echo BASE_URL.'/about.php'?>">About us</a></li>
            <li><a href="<?php echo BASE_URL.'/plans.php'?>">Plans</a></li>
            <li><a href="<?php echo BASE_URL.'/faqs.php'?>">FAQ's</a></li>
            <li><a href="<?php echo BASE_URL.'/terms.php'?>">Terms</a></li>
            <li><a href="<?php echo BASE_URL.'/contact.php'?>">Contact us</a></li>
            <li><a href="<?php echo BASE_URL.'/register/signin.php'?>">Login</a></li>
            <li><a class="register" href="<?php echo BASE_URL.'/register/signup.php'?>">Register</a></li>
        </ul>
        
        <label class="menu-btn" id="menu-btn">
            <span id="menu-btn1"></span>
            <span id="menu-btn2"></span>
            <span id="menu-btn3"></span>
        </label>  
    </header>
    
    <div class="under-header"></div>

     <!--Start of Tawk.to Script-->
 