...
if(isset($_POST['deposit'])){
    ...
    if(empty($_FILES['reference']['name'])){
        $errors['reference'] = "Please select payment screenshot";
    }
...
if(isset($_POST['cashout'])){
    ...
    if($_POST['amount'] <= 199){
        $errors['amount'] = "Minimum Cashout allowed is $200";
    }
    ...
    if (count($errors) === 0){
        ...
        if (count($errors) === 0){
            ...
            if($_POST['amount'] > 600){
                if($confirmedReferrals < 9){
                    $errors['referrals'] = "You have no referrals";
                }
            }

            if($confirmedCashouts >= 300 && $_POST['amount'] > 200){
                if($confirmedReferrals < 9){
                    $errors['referrals'] = "You have no referrals";
                }
            }
...
if(isset($_POST['invest'])){
    ...
    if($_POST['amount'] <= 199){
        $errors['amount'] = "Minimum Investment allowed is $200";
    }
    ...
    $existinginvestments = selectAll('transactionz', ['user_id'=> $_SESSION['id'], 'type'=>'investment']);

    foreach($existinginvestments as $existinginvestment){
        if($existinginvestment['payment_method'] < 7){
            $errors['Multiple investment'] = "Multiple investment, Contact Customer care";
        }
    }
